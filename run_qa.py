...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Cap the number of PDF map pages (preview). Default: all configured flags.",
    )
    ...
    mapped_flags = [f for f in flags if f["flag_type"] in pdf_types]
    skipped = len(flags) - len(mapped_flags)

    if args.max_pages is not None and len(mapped_flags) > args.max_pages:
        ...
        print(
            f"PREVIEW: generating {len(mapped_flags)} of {len(flags) - skipped} "
            f"map-eligible flags (sampled across types)..."
        )
    else:
        print(
            f"Generating {len(mapped_flags)} map pages "
            f"({skipped} bulk flags are CSV-only)..."
        )
    generate_qa_maps(
        pipes_repaired,
        manholes,
...
def _write_flags_csv(flags: list[dict], path: str) -> None:
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["flag_type", "severity", "pipe_id", "x", "y", "description"],
        )
        writer.writeheader()
        for flag in flags:
            geom = flag["geometry"]
