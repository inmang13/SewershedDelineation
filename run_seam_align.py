...
from boundary import align_seams, enforce_containment, bridge_parts  # noqa: E402
...
def main():
    ...
    ap.add_argument("--contain", type=float, default=0.8,
                    help="overlap fraction of the smaller polygon that makes a "
                         "pair 'nested' for exact containment")
    args = ap.parse_args()
    ...
    aligned = enforce_containment(aligned, min_frac=args.contain)
    ...
    print("\nPer-site IoU (before -> after seam align + containment):")
    ...
    # --- containment check on known nested pair --------------------------
    for sid_in, sid_out in [("31067", "29962")]:   # Tract 3.01 inside 1.02
        if sid_in in aligned and sid_out in aligned:
            leak = aligned[sid_in].difference(aligned[sid_out]).area
            print(f"\nContainment {tract.get(sid_in)} within {tract.get(sid_out)}: "
                  f"outside area {leak:.1f} sq ft "
                  f"({'exact' if leak < 1 else 'NOT contained'})")
